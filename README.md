...
- **Tailwind CSS 4 全新界面** — 深浅主题（含跟随系统）、移动端适配（底部胶囊播放器 + 底部导航 + 抽屉式队列）、自建通用组件库（BaseModal / BaseButton 等）、PWA Service Worker 离线缓存与安装引导
- **TS 服务器频道树** — WebUI 侧边抽屉浏览完整频道树与在线用户，管理员点击频道一键移动机器人
- **播放队列拖拽重排序** — 鼠标 / 触屏拖拽，配合 `!reorder <from> <to>` 聊天命令；队列抽屉打开自动定位当前播放行；行内显示点歌人，「正在播放」高亮按播放实例精确匹配（上游按歌曲 id 匹配，同名重复曲目会全部点亮）
- **歌词页重构** — 单轴原生滚动（跟手，已播歌词可回滚）；手动浏览时暂停自动跟随，分割线 + ▶时间胶囊指示视口位置、点击即跳转（无播放权限时给出提示）；上下留白随视口高度动态计算；译文开关、三档字号
- **移动端胶囊播放器** — 封面 / 歌名点击直达歌词页，44px 触控控制键，切歌与 seek 触感反馈；移动端输入框防 iOS 聚焦缩放
- **断线全局横幅** — WebSocket 断连即时可见（重连中 / 重试耗尽两态），一键手动重连
- **信息架构重组**（v2.1.0）— 收藏 / 历史并入「我的」页子 Tab；导航收敛为 发现 / 搜索 / 我的 / 设置（游客仅前两个）；设置页重组为 8 个分区并支持改动自动保存（失败自动回滚）；宽屏内容约束；首页按角色给出无机器人引导
- **队列与收藏操作**（v2.1.0）— 队列抽屉拆分「清空队列」（保留当前曲继续播）与「停止」；收藏页按筛选结果「全部加进队列」（上限 100 首）
...
- **长暂停后恢复自动重取播放链接**（v2.2.1）— 临时播放链接（网易等）在长暂停期间过期后，恢复时自动重取一条新链接并按暂停位置重建播放管线，修复此前恢复只播几秒残余缓冲就静音直到切歌的问题（过期 URL 下 FFmpeg 对 403 无限重连、永不退出也不报错，复用旧链接的兜底必然失败）
...
- **Docker 发版流水线** — 一条流水线完成完整发版：测试门槛 → 推送多架构镜像到 GHCR（`x.y.z` / `latest`）→ Release 挂载 amd64/arm64 双架构离线 tar.gz。部署与上游同构：`docker-compose pull && docker-compose up -d` 即可运行 / 升级，无需本地编译；离线服务器可下载 Release 离线包 `docker load` 后配合 `docker-compose.prod.yml` 启动。**从上游镜像切换到本 fork**：`data/` 数据目录（数据库、配置、Cookie）同源兼容，镜像渠道改为 `ghcr.io/wslinnn/teamspeak-music-bot`，切换即可沿用数据（建议操作前备份）
...
- **游客模式（免登录点歌，默认关闭）** — 管理员可选择允许访客**无需账号密码**进入 WebUI 点歌，并逐项配置游客权限（8 个开关，默认全部关闭，需逐项放开）与可控机器人白名单；游客无法查看 / 修改任何设置、管理机器人或访问用户管理。开启后登录页出现 **「以游客身份进入」**。详见下文 **「游客模式 / Guest mode」** 小节
...
- **频道无人时自动暂停** — 机器人所在频道没有其他人时自动暂停播放，有人回到频道后自动恢复（**默认关闭**，可在设置中开启）。恢复有三条触发路径：进入频道的推送事件、频道内的语音活动（有人说话立即恢复，UDP 路径、不依赖事件通知）、以及自动暂停期间每 30 秒一次的对账兜底。手动暂停不会被自动恢复顶开
...
- **服务端内容缓存** — 歌词 / 歌单 / 专辑 / 歌曲详情 / 搜索 / 推荐歌单榜经 LRU+TTL 缓存（歌词永续、歌单专辑等 1h、搜索 10min、空结果 10min，容量 600 条/音源），多客户端同歌切歌只穿透一次上游，WebUI/桌面端/`!lyrics` 命令共享命中；播放流地址**不**缓存（URL 自带过期）。详见 [docs/server-cache-plan.md](docs/server-cache-plan.md)
- **桌面客户端鉴权通道（`/api/client`）** — 面向桌面端伴侣应用等非浏览器客户端的 Bearer token 通道：登录换 token（60 天有效、每账号最多 10 台设备、超出自动挤掉最旧的），改密码 / 管理员重置 / 删除用户即时失效全部 token；REST 与 WebSocket 同通道鉴权，与 WebUI 的 cookie 会话完全隔离互不影响
...
- **逐项权限（8 个开关，管理员配置）**：v2.1.0 起**全部默认关闭**（此前「添加到队列末尾」默认开启，未显式配置过的部署升级后需按需重新放开）。

  | 开关 | 字段 | 默认 |
  |------|------|------|
  | 添加到队列末尾 | `addToQueue` | 关 |
  | 添加到下一首 | `playNext` | 关 |
...
**`config.adminGroups`（现已启用）**：用于限制管理类聊天命令（`stop`/`clear`/`remove`/`move`/`vol`/`mode`）只能由指定 TeamSpeak 服务器组的成员运行；为空时不做任何限制（向后兼容）。详见 [TeamSpeak 命令权限](#teamspeak-命令权限管理类命令限制)。`config.adminPassword` 已废弃，当前版本不再写回配置文件（历史文件中的明文旧值会在下一次保存时自动移除），可以放心忽略。
...
> **关于 `adminPassword` 和 `adminGroups`**：`adminGroups` 现已启用，用于限制管理类聊天命令只能由指定 TeamSpeak 服务器组运行（为空 = 不限制），详见 [TeamSpeak 命令权限](#teamspeak-命令权限管理类命令限制)。`adminPassword` 已废弃且**不再写回配置文件**（历史文件里的明文旧密码会在下一次保存配置时自动移除）——WebUI 鉴权改为基于数据库的用户账号系统（见 [首次配置](#首次配置)），无需在 `config.json` 中设置密码。
...
- **`webHost`（默认 `"0.0.0.0"`，监听所有网卡）**：只在本机或本机反代访问 WebUI 时，建议设为 `"127.0.0.1"`，把管理面从局域网里收回来。
- **Windows 文件权限**：`data/` 下的 `config.json`（含各平台明文密码/密钥）、SQLite 数据库与 cookies 全部只做 POSIX 0600 权限，**Windows 会忽略权限位**，实际保护依赖目录 ACL。自托管在多账号机器上时，请手动收紧 `data\` 目录的 ACL（仅保留当前用户与 SYSTEM）。

> **安全提示（X-Forwarded-For 伪造）**：`"trustProxy": true` 会信任请求头里整条 `X-Forwarded-For` 链。如果应用端口除了代理之外还能被直接访问（例如同时监听公网、或代理与本机之间没有防火墙隔离），攻击者可以每次请求伪造一个不同的 `X-Forwarded-For`，绕过按 IP 键控的登录限流（登录限流已额外按提交的用户名聚合，单账号爆破仍会被拦住，但 IP 维度的其它限制仍可能被轮换头绕过）。因此启用 `trustProxy` 时，请务必用防火墙/安全组限制应用端口，**只允许反向代理的来源 IP 访问**。
...
**Q：机器人播放时突然自动暂停了？**
A：这是"频道无人时自动暂停"功能：当机器人所在频道没有其他人时会自动暂停，有人加入后自动恢复，避免空播。该功能**默认关闭**，仅在你于 **设置 → 行为设置** 开启后生效；如需停用，在同一页面关闭即可。（占用判定基于进出频道事件，并每 30 秒与服务器对账一次；恢复播放有三条触发路径：有人进入频道的推送事件、频道内有人说话的语音活动，以及 30 秒一次的对账兜底。手动暂停不会被自动恢复顶开。）
...
修复一个播放稳定性缺陷。**没有配置变化，没有破坏性改动**，升级照常拉取重启即可。

**问题现象**

- 暂停一段时间（手动暂停或频道无人自动暂停均可触发）后再恢复播放，机器人只播出几秒残余缓冲就一路静音，WebUI 进度条冻结，直到约一分钟后 stall 看门狗触发切歌——当前曲没有播完的部分被静默跳过。暂停期间网易等平台的临时播放链接过期后必然复现。

**根因**

- 恢复时 FFmpeg 进程通常仍然存活：长暂停期间 CDN 侧早已掐断闲置连接，恢复后 FFmpeg 会对过期的播放 URL 无限重连（对 403 反复重试，**永不退出也不报错**，实测确认），恢复逻辑只是对着一个已死的上游继续发包。
- 此前的兜底（检测到 FFmpeg 已死时按暂停位置重启当前曲）复用的也是**同一条过期 URL**，且"检测到失败就重新获取播放链接"在实现中从未落地——全代码库只有新歌起播时会请求播放链接。

**修复**

- 恢复 URL 音源前，若当前播放链接签发已超过 5 分钟，自动重新请求一条新链接，并以暂停位置为偏移重建播放管线；接入 `!resume` 命令、频道占用自动恢复与设置保存三处入口。
- Spotify 走 sidecar 自管传输不适用此路径；链接尚年轻时保持原有即时恢复（残余缓冲立即出声）；重取失败回退原恢复路径，最坏情况与旧版本行为一致。
- 暂停期间拖动进度条的场景无需单独处理：恢复重建后链接已刷新，seek 产生的坏管线会被随后的恢复替换。
- 阈值取 5 分钟的考量：各平台链接有效期未知且接口不返回，量级为几十分钟；取远低于下限的值，误判的代价只是一次廉价的链接解析请求，不会产生可感知的播放延迟。

### v2.2.0：桌面客户端支持 / 服务端内容缓存

为桌面端伴侣应用（tsmb-desktop，Tauri 2 + Rust，独立仓库）打地基，顺带把多客户端场景的上游压力收掉。**没有配置变化，没有破坏性改动**：WebUI 的 cookie 会话鉴权、全部设置项与播放行为完全不变，升级照常拉取重启即可。

**`/api/client` Bearer token 鉴权通道**

- 面向非浏览器客户端的新登录端点：`POST /api/client/login` 用账号密码换 token（随设备名入库），之后 REST 与 WebSocket 一律走 `Authorization: Bearer` 头，与 WebUI 的 cookie 会话**完全隔离**——桌面端登录不影响网页会话，反之亦然。
- token 有效期 60 天，每账号最多 10 台设备，超出自动挤掉最旧的；五条失效路径全部即时生效并断开对应 socket：过期、客户端主动登出（`DELETE /api/client/session`）、用户改密码、管理员重置密码、删除用户。
- 安全设计：token 只存 SHA-256 哈希（泄露数据库不等于泄露 token）；Bearer 请求明确豁免 CSRF Origin 检查（自定义头不是浏览器会自动附带的环境凭据，跨站页面无法伪造）；注销 / 失效只精确关闭该 token 自己的 WebSocket，不影响同账号其他设备。
- 受限成员的能力与机器人白名单在此通道同样生效：REST 逐请求校验，WS 在升级时按同一权限上下文盖章过滤可见机器人。

**Web 端 seek 补发 stateChange**

- 在 WebUI 拖动进度条后，服务端现在会向所有 WS 客户端广播一次 stateChange。此前桌面端歌词要等下一轮轮询才追上，现在即时收敛。

**服务端内容缓存**

- 歌词 / 歌单 / 专辑 / 歌曲详情 / 搜索 / 推荐歌单榜走 LRU+TTL 缓存（策略与内存上限见 [docs/server-cache-plan.md](docs/server-cache-plan.md)），多客户端听同一首歌时上游只被请求一次；WebUI、桌面端、`!lyrics` 命令共享命中。
- `/api/music/lyrics/:id` 补上路由级限流（此前是音乐类接口中唯一没有的），防止单 IP 滥用穿透上游。
- 播放流地址**有意不缓存**（URL 自带过期时间，缓存会导致播放失败）；私人 FM / 每日推荐等动态接口也不缓存。
...
- **配置变更**：反向代理部署务必 `"trustProxy": true`（详见 [反向代理部署注意事项](#反向代理部署注意事项)）。`config.adminGroups` 现已启用，用于限制管理类聊天命令只能由指定 TeamSpeak 服务器组运行（为空 = 不限制，详见 [TeamSpeak 命令权限](#teamspeak-命令权限管理类命令限制)）；`config.adminPassword` 已废弃，不再写回配置文件。
