...
- **频道无人自动暂停 + 有人回来自动恢复** — 占用判定事件源化（自机频道显式解析，不依赖多人服务器上易超时的 `clientlist` 全量查询）；恢复有三条触发路径：进入频道的推送事件、频道内语音活动（UDP，秒级）、30 秒一次的对账兜底；手动暂停不会被自动恢复顶开；长暂停后从暂停位置偏移重启续播。**v2.1.0 修复了此前版本占用判定因协议库缺陷整体失效的问题**（该功能在 v2.0.0 上实际不可用）
