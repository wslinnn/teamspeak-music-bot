...
- ✅ Step 0–4 全部完成：dev 分支 12 个提交，1049/1049 测试全绿，已推送 origin/dev
- ✅ Step 5 完成：CI 预发布标记、README fork 段、FORK.md、main 维护公告 + legacy tag 已推送
- ✅ Step 6 存量演练通过（在 data/ 副本上，原始库零污染）：
  - first-run 创建管理员 → 登录/登出/错误密码 401 全部正确
  - 存量收藏（2 首）经 /api/song-favorites 完好可见
  - 机器人用存量身份自动连上真实 TS 服务器（协议兼容确认）
  - 频道树端点返回真实频道/客户端；reorder 空队列 400 语义正确
  - 网易云内嵌 API 正常，真实搜索 80 条结果；CSRF 对无 Origin 请求 fail-closed
- ⏭ 待办：真实试用期（日常使用 1–2 周）→ Step 7 交接（main 重置到 dev）
- 备注：上游 `loadSavedBots()` 会自动连接已保存机器人，演练/迁移时注意与生产实例的会话冲突
