...
> ✅ **状态：8 个方案已全部实现**（2026-08-24，9 个提交，含一个实现中发现的修复：`/api/bot/settings` 补 commandPrefix 读写——上游无此端点导致命令前缀设置一直静默失效）。各方案按原设计落地，仅两处与文档有出入：闲置超时因已在通用 Tab 而未在行为 Tab 重复；搜索「加载更多」仅单平台模式提供（/search/all 无 offset 分页）。
